...
- 已实现用户数据卡片列表展示，采用手机端卡片布局
- 已新增公共组件 `src/components/UserCard.tsx`
- `查看用户` 页面已接入 `src/data/data.json` 静态数据
- 已实现分页功能，每页展示 10 条记录
- 页面顶部已显示总数据条数，底部已提供「上一页 / 下一页」和页码显示
- 已为用户卡片增加折叠/展开功能，折叠态仅显示姓名、用户编号、性别
- 已实现“每页首张卡片默认展开，其余默认折叠”的引导逻辑
- 已在页面右下角增加毛玻璃风格的返回顶部按钮

#### Step 6：风险评估 — 数据筛选

- 已实现基于 2026 年规则的风险用户筛选逻辑
- 当前筛选结果共 `700` 名风险用户
- 风险评估页面已支持分页展示，每页 10 条
- 风险用户卡片已增加红色左边框标识风险状态
- 已复用 `UserCard.tsx` 展示风险用户完整信息
- 风险评估页已同步支持折叠/展开与返回顶部按钮

#### Step 7：风险评估 — 搜索功能

- 已在风险评估页面顶部增加身份证搜索框
- 已支持点击搜索按钮或按回车进行查询
- 搜索命中时会高亮展示完整风险用户信息，并提示「⚠️ 该用户在风险名单中」
- 未命中时会显示「✅ 未在风险名单中找到该用户」
- 已增加清除按钮，点击后可恢复完整风险列表与分页
...
- 已验证「查看用户」第一页展示 10 条记录，翻页到第二页后内容和页码同步更新
- 已验证折叠卡片、翻页后默认展开策略以及返回顶部按钮均正常工作
...
- 继续实现 Step 8：查询数据功能
- 可复用 `UserCard.tsx`，并在查询页中区分身份证精确匹配与姓名模糊匹配
...
### Step 5：查看用户功能
- **状态：** 已完成（2026-03-19）
- **新增文件：** `src/components/UserCard.tsx`
- **备注：** 已完成用户卡片列表、总条数展示、每页 10 条分页、折叠卡片和返回顶部按钮

### Step 6：风险评估 — 数据筛选
- **状态：** 已完成（2026-03-19）
- **新增文件：** 无
- **备注：** 已完成风险用户筛选、风险列表分页、红色风险标识，以及折叠卡片与返回顶部按钮复用

### Step 7：风险评估 — 搜索功能
- **状态：** 已完成（2026-03-19）
- **新增文件：** 无
- **备注：** 已完成身份证搜索、命中高亮、未命中提示和清除恢复列表
